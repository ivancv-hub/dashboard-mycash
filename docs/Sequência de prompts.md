...
Antigravity, antes de começarmos a implementação, preciso que você faça uma análise completa do projeto:
...
🧭 Antigravity — Modo Execução Semi-Autônoma Guiada por Aprovação
...
Rodar npm run build (corrigir até passar)
Informar o que foi implementado
Parar e aguardar aprovação (“Fazer commit e documentar” ou ajustes)
Atualizar DOCUMENTATION.md
Depois do commit perguntar se pode seguir para o próximo prompt
...
Antigravity, agora vamos criar a estrutura base do projeto:
...
Antigravity, vamos implementar o sistema de navegação desktop com a sidebar:
...
Antigravity, agora vamos criar a versão mobile da navegação:
...
Antigravity, vamos criar os três cards de resumo que aparecem no topo do dashboard:
...
Antigravity, vamos implementar a barra de controles no topo do dashboard:
...
Antigravity, vamos criar o widget de categorias com gráficos donut:
...
Antigravity, vamos criar o gráfico de evolução de receitas e despesas:
...
Implemente tooltip interativo: quando o mouse se move sobre o gráfico, uma linha vertical fina cinza clara acompanha o Antigravity. Ao parar sobre um ponto, mostre tooltip flutuante com fundo branco, sombra elevada e bordas arredondadas.
...
Antigravity, vamos criar o widget que exibe os cartões de crédito:
...
Implemente interatividade: ao passar mouse sobre um card, ele eleva levemente no eixo vertical (translateY -4px ou -8px) acompanhado de aumento sutil da sombra. Transição suave de 200-300ms. Antigravity muda para ponteiro indicando clicabilidade.
...
Antigravity, vamos criar o widget de próximas despesas com lista cronológica de contas a pagar:
...
Antigravity, vamos criar a tabela completa de transações no dashboard:
...
Botões Anterior/Próxima ficam disabled (cor cinza clara, Antigravity normal) quando não aplicáveis (primeira/última página respectivamente).
...
Antigravity, vamos criar o modal completo para adicionar transações:
...
Antigravity, vamos criar o modal para adicionar membros da família:
...
Antigravity, vamos criar o modal para adicionar contas bancárias e cartões de crédito:
...
Antigravity, vamos criar o modal que mostra informações completas do cartão:
...
Antigravity, vamos criar o modal de filtros específico para mobile:
...
Antigravity, vamos criar a tela completa dedicada aos cartões de crédito:
...
Antigravity, vamos criar a tela completa dedicada às transações:
...
Antigravity, vamos criar a tela de perfil do usuário:
...
Antigravity, agora vamos criar a aba "Configurações" dentro da view de perfil:
...
Antigravity, vamos implementar animações e transições suaves em todo o sistema:
...
Antigravity, vamos criar funções utilitárias para formatação consistente em todo o sistema:
...
Antigravity, faça uma revisão completa de responsividade do sistema já implementado, aplicando apenas ajustes incrementais de layout, sem refatorar arquitetura, lógica de negócio ou recriar componentes existentes.
...
Antigravity, vamos finalizar o projeto com testes e validação completa do sistema:
Crie fluxo de teste completo simulando jornada de usuário real:
Usuário abre o sistema pela primeira vez
Vê dados mock pré-carregados no dashboard
Clica em um membro da família para filtrar
Verifica que todos os cards, gráficos e tabela atualizam mostrando apenas dados daquele membro
Clica novamente no membro para remover filtro
Abre o seletor de período e escolhe "Últimos 3 meses"
Verifica que dados atualizam para mostrar 3 meses
Digita texto no campo de busca
Verifica que tabela filtra em tempo real
Clica em "Nova Transação"
Preenche formulário completo com todos os campos
Salva transação
Verifica que modal fecha, toast aparece, e nova transação aparece na tabela
Clica em cartão no widget
Modal de detalhes abre mostrando informações corretas
Navega para "Cartões"
Verifica que todos os cartões aparecem com informações corretas
Navega para "Transações"
Usa filtros avançados
Verifica resultados
Navega para "Perfil"
Vê informações do usuário e membros da família
Troca para aba "Configurações"
Interage com toggles e configurações
...
Antigravity, vamos fazer a revisão final e preparação para entrega do projeto mycash+:
