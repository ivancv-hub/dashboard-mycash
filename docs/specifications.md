...
Colunas da Tabela:
Avatar: Coluna estreita (50px) mostrando foto circular pequena (24px) do membro responsável pela transação. Se não houver foto ou membro, mostra ícone de usuário genérico.
Data: Mostra data da transação formatada como "DD/MM/AAAA" (exemplo: "15/01/2024") em texto cinza médio.
Descrição: Mostra ícone indicativo do tipo seguido da descrição textual. Para receitas, ícone é seta diagonal para baixo-esquerda em círculo com fundo verde claro. Para despesas, ícone é seta diagonal para cima-direita em círculo com fundo vermelho claro. A descrição aparece em texto negrito preto.
Categoria: Nome da categoria em badge arredondado com fundo cinza claro e texto cinza médio.
Conta/Cartão: Nome da conta bancária ou cartão de crédito de onde saiu/entrou o dinheiro. Texto cinza médio. O sistema busca o nome na lista de contas; se não encontrar, busca na lista de cartões; se não encontrar em nenhuma, mostra "Desconhecido".
Parcelas: Se a transação foi parcelada, mostra "3x", "6x", etc. Se foi à vista (parcelas = 1), mostra apenas "-" (traço).
Valor: Alinhado à direita, mostra o valor com prefixo de sinal. Receitas têm "+" em verde ("+R$ 5.000,00"). Despesas têm "-" em preto ("-R$ 350,00"). Fonte negrito.
...
Modal abre com fade-in (200ms)
Formulário renderiza com campos vazios/padrão (Tipo: Despesa, Parcelas: 1, Status: Concluído)
Usuário seleciona tipo "Receita" → botão fica preto
Usuário digita valor "5000" → campo formata automaticamente para "R$ 5.000,00"
Usuário digita descrição "Salário do mês"
Usuário clica no select de categoria → dropdown abre mostrando apenas categorias de receita
Usuário escolhe "Salário" → dropdown fecha, valor aparece no select
Usuário clica no campo de data → calendário abre
Calendário mostra mês atual com dia atual destacado
Usuário clica em um dia → data é selecionada, calendário fecha
Usuário clica no select de conta → dropdown abre com todas contas e cartões
Usuário escolhe "Itaú Conta Corrente"
Usuário deixa membro vazio (transação familiar)
Usuário deixa parcelas em 1 (à vista)
Usuário deixa status em Concluído
Usuário clica em "Salvar Transação"
Sistema valida todos os campos → todos válidos
Sistema cria objeto: { id: "uuid-generated", type: "income", amount: 5000, description: "Salário do mês", category: "Salário", date: Date object, accountId: "itau-cc-id", memberId: null, installments: 1, status: "completed" }
Sistema adiciona objeto ao array de transações no contexto global
Modal fecha com fade-out (200ms)
Toast aparece no canto superior direito: "Transação adicionada com sucesso!" em verde
Card de Receitas no dashboard recalcula: soma todas receitas incluindo a nova → atualiza valor exibido com animação de contagem
Card de Saldo Total recalcula: adiciona R$ 5.000 ao saldo anterior → atualiza com animação
Gráfico de categorias NÃO muda (pois nova transação é receita, gráfico mostra apenas despesas)
Tabela de transações adiciona nova linha no topo (por ser mais recente)
Linha aparece com fade-in
Se tabela tinha paginação e estava cheia, pode adicionar nova página
Contador da paginação atualiza: "Mostrando 1 a 5 de 48" (era 47, agora é 48)
Toast desaparece automaticamente após 3 segundos com fade-out
Fluxo: Filtrar por Membro e Período Combinados
Dashboard está mostrando dados consolidados de toda a família do mês atual
Usuário clica no avatar de "Maria" nos filtros do header
Avatar de Maria ganha borda preta e ícone de check
Filtro de membro "Maria" é aplicado ao estado global
Sistema dispara recálculo em todos os componentes que dependem de transações:
Cards de Resumo:
...
Todas as atualizações acontecem simultaneamente com animações coordenadas (duração similar)
Agora usuário decide alterar o período
Usuário clica no botão de período (que mostra mês atual)
Calendário duplo abre
Usuário clica em dia 1 de Janeiro como início
Intervalo começa a ser definido (visual de seleção temporária)
Usuário clica em dia 31 de Março como fim
Intervalo completo fica destacado (3 meses)
Usuário clica fora do calendário ou em OK
Calendário fecha
Filtro de período "01/01/2024 - 31/03/2024" é aplicado ao estado global
Texto do botão de período atualiza: "01 jan - 31 mar, 2024"
Sistema dispara novo recálculo em todos os componentes, agora com DOIS filtros ativos:
...
Todas as visualizações atualizam novamente com animações
Dados agora refletem apenas Maria nos últimos 3 meses
Se usuário clicar novamente no avatar de Maria, o filtro de membro é removido mas o filtro de período permanece
Dados voltam a mostrar toda a família, mas ainda limitados aos últimos 3 meses
Fluxo: Interação com Calendário de Contas
Widget de calendário renderiza mostrando o mês atual
Sistema busca todas as contas pendentes (status = "pending")
Para cada conta, verifica se a data de vencimento cai no mês sendo exibido
Conta 1: "Conta de Luz", vence dia 15 do mês atual → adiciona indicador visual no dia 15
Conta 2: "Internet", vence dia 17 do mês atual → adiciona indicador no dia 17
Conta 3: "Escola", vence dia 20 do mês atual → adiciona indicador no dia 20
Calendário renderiza com três dias tendo ponto vermelho abaixo do número
Por padrão, o dia atual (hoje) está selecionado
Lista abaixo mostra contas do dia atual
Hoje não há contas → lista mostra "Nada hoje." com borda tracejada
Usuário clica no dia 15
Dia 15 fica destacado com fundo verde-limão
Lista abaixo atualiza: busca contas com data = dia 15
Encontra "Conta de Luz - R$ 240,00"
Lista renderiza um item:
Indicador circular vermelho (pendente)
Texto "Conta de Luz"
Botão de check à direita
Usuário passa mouse sobre o botão de check
Botão muda: fundo fica verde claro, borda verde, ícone verde
Usuário clica no botão de check
Sistema altera o status da conta no array de contas: { ...conta, paid: true }
Indicador circular muda de vermelho para verde instantaneamente
Ponto vermelho abaixo do dia 15 no calendário desaparece (já não há contas pendentes naquele dia)
Lista permanece mostrando a conta (agora em verde) por alguns segundos
Usuário clica no dia 17
Lista atualiza mostrando "Internet - R$ 120,00" com indicador vermelho
Dia 17 tem ponto vermelho
Processo se repete
...
Somar saldos positivos de todas as contas bancárias cadastradas
Para cada cartão de crédito, verificar o valor da fatura atual
Subtrair todos os valores de faturas do total de contas
Resultado: Saldo Total = Σ(saldos de contas) - Σ(faturas de cartões)
Exemplo:
...
Filtrar array de transações por tipo = "income"
Se há filtro de período ativo, manter apenas transações cuja data está dentro do intervalo
Se há filtro de membro ativo, manter apenas transações daquele membro
Somar o campo "amount" de todas as transações restantes
Retornar soma
...
Filtrar despesas daquela categoria específica
Aplicar filtros ativos (período, membro, busca)
Somar valores: valorTotalCategoria
Calcular receita total do período (com mesmos filtros)
Percentual = (valorTotalCategoria / receitaTotal) × 100
Arredondar para 1 casa decimal
Se receita total for zero, retornar 0% para evitar divisão por zero
...
Criar array de objetos: [{ nome, valor, percentual }, ...]
Ordenar array usando sort() com comparação: (a, b) => b.valor - a.valor
Isso coloca categoria com maior valor primeiro, menor valor por último
...
Pegar valorDaFatura e limiteTotal do cartão
Se limiteTotal for zero, retornar "0.0%" para evitar divisão por zero
Caso contrário: percentual = (valorDaFatura / limiteTotal) × 100
Formatar com 1 casa decimal: "69.0%"
Exibir no badge
...
Pegar valorAtual e valorMeta do objetivo
Calcular: percentual = (valorAtual / valorMeta) × 100
Aplicar Math.min(percentual, 100) para garantir máximo de 100%
Arredondar para número inteiro para display: "35%"
Calcular valorRestante = valorMeta - valorAtual
Se valorRestante for negativo (objetivo ultrapassou meta), mostrar zero
Formatar valorRestante como moeda: "Faltam R$ 6.500,00"
